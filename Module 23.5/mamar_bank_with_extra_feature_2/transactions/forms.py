...
class TransferBalanceForm(TransactionForm):
    ...
    def clean_amount(self):
        account = self.account # user er BankAccount
        min_transferrable_amount = 50
        max_transferrable_amount = 100000
        balance = account.balance
        amount = self.cleaned_data.get('amount')  # user er fillup kora form theke amra amount field er value ke niye ashlam! (Example: 500)
        if amount < min_transferrable_amount:
            raise forms.ValidationError(
                f'You can transfer at least {min_transferrable_amount} $'
            )
        if amount > max_transferrable_amount:
            raise forms.ValidationError(
                f'You can withdraw at most {max_transferrable_amount} $'
            )
        if amount > balance: # amount = 5000, tar balance ache 200
            raise forms.ValidationError(
                f"You've {balance} $ in your account. "
                "You can't transfer money more than your account balance"
            )
        return amount